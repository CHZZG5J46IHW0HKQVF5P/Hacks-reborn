...
int isBadPtr_GTA_pVehicle ( vehicle_info *p_VehicleInfo )
{
	...
	if ( !
			(
				(DWORD) p_VehicleInfo >= (DWORD) Stuff::getVehiclesPool()->start
				&& (DWORD) p_VehicleInfo <= ((DWORD)Stuff::getVehiclesPool()->start + (Stuff::getVehiclesPool()->size * sizeof(vehicle_info)))
			)
		)
		return 1;
...
int isBadPtr_GTA_pPed ( actor_info *pActorInfo )
{
	...
	if ( !
			(
				(DWORD) pActorInfo >= (DWORD) Stuff::getActorsPool()->start
				&& (DWORD) pActorInfo <= ((DWORD)Stuff::getActorsPool()->start + (Stuff::getActorsPool()->size * sizeof(actor_info)))
			)
		)
		return 1;
...
int isBadPtr_GTA_pBuildingInfo ( DWORD p_BuildingInfo )
{
	...
	struct Stuff::pool *pool_building = *(struct Stuff::pool **)BUILDING_POOL_POINTER;
...
int isBadPtr_GTA_pObjectInfo ( DWORD p_ObjectInfo )
{
	...
	struct Stuff::pool *pool_object = *(struct Stuff::pool **)OBJECT_POOL_POINTER;
...
struct actor_info *actor_info_get ( int id, int flags )
{
	...
	if ( Stuff::getActorsPool() == NULL )
		return NULL;

	if ( id != ACTOR_SELF && (id < 0 && id >= Stuff::getActorsPool()->size) )
		return NULL;
	...
	{
		info = &( (struct actor_info *)Stuff::getActorsPool()->start )[id];
		if ( isBadPtr_GTA_pPed(info) )
			return NULL;
	}
...
struct vehicle_info *vehicle_info_get ( int id, int flags )
{
	...
	if ( Stuff::getVehiclesPool() == NULL )
		return NULL;

	if ( id != VEHICLE_SELF && (id < 0 && id >= Stuff::getVehiclesPool()->size) )
		return NULL;
	...
	{
		info = &( (struct vehicle_info *)Stuff::getVehiclesPool()->start )[id];
		if ( isBadPtr_GTA_pVehicle(info) )
			return NULL;
	}
...
int actor_find_nearest_ex(int flags, std::function<bool(actor_info *)> pred)
{
	...
	for ( n = 0; n < Stuff::getActorsPool()->size; n++ )
	{
...
{
	return (int)( (DWORD) vehicle - (DWORD) Stuff::getVehiclesPool()->start ) / 2584;
}
int getPedGTAIDFromInterface ( DWORD *ped )
{
	return (int)( (DWORD) ped - (DWORD) Stuff::getActorsPool()->start ) / 1988;
}
