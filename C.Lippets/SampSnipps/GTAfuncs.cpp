...
{
	if (*PEDSELF->GetMemoryValue(0x530) == 50)
		PEDSELF->GetVehicle()->SetPosition(pos[0], pos[1], pos[2]);
	else
		PEDSELF->SetPosition(pos[0], pos[1], pos[2]);
}
...
{
	DWORD *actor = (DWORD *)SF->getGame()->actorInfoGet(ACTOR_SELF, 0);
...
{
	if (isBadPtr_GTA_pVehicle(vehicle))
		return;
	vehicle_info* pVehicleInfo = vehicle_info_get(VEHICLE_SELF, 0);

	if (pVehicleInfo)
	{
		CVehicle *pCurrentVehicle = GAME->GetPools()->GetVehicle((DWORD *)pVehicleInfo);
		if (pCurrentVehicle)
		{
...
void GTAfunc_PutActorInCarAsPassenger(vehicle_info *vehicle, int iSeat)
{
	...
	if (pVehicle)
	{
		CTaskSimpleCarSetPedInAsPassenger* pInTask = GAME->GetTasks()->CreateTaskSimpleCarSetPedInAsPassenger(pVehicle, iSeat);
		if (pInTask)
		{
			pInTask->SetIsWarpingPedIntoCar();
			pInTask->ProcessPed(PEDSELF);
...
void GTAfunc_EnterCarAsDriver(vehicle_info *vehicle)
{
	...
	if (pVehicle)
	{
		CTaskComplexEnterCarAsDriver* pInTask = GAME->GetTasks()->CreateTaskComplexEnterCarAsDriver(pVehicle);
		if (pInTask)
			pInTask->SetAsPedTask(PEDSELF, TASK_PRIORITY_PRIMARY, true);
	}
}
...
void GTAfunc_CameraOnActor(actor_info *actor)
{
	...
	CEntity *pEntity = GAME->GetPools()->GetEntity((DWORD *)actor);
...
void GTAfunc_CameraOnVehicle(vehicle_info *vehicle)
{
	...
	CEntity *pEntity = GAME->GetPools()->GetEntity((DWORD *)vehicle);
	CCamera *pCamera = GAME->GetCamera();
	if (pCamera && pEntity)
...
{
	CAnimBlock *pBlock = GAME->GetAnimManager()->GetAnimationBlock(szBlockName);
	if (pBlock)
	{
		bool bLoaded = true;
		...
		if (!pBlock->IsLoaded())
		{
			...
			GAME->GetStreaming()->RequestAnimations(pBlock->GetIndex(), 4);
		...
		if (bLoaded)
		{
			...
			PEDSELF->GetPedIntelligence()->GetTaskManager()->Flush(TASK_PRIORITY_PRIMARY);

			// removes temporary tasks caused by events like falling
			PEDSELF->GetPedIntelligence()->GetTaskManager()->RemoveTask(TASK_PRIORITY_EVENT_RESPONSE_TEMP);

			// remove jumping task
			CTask *jumpTask = PEDSELF->GetPedIntelligence()->GetTaskManager()->FindActiveTaskByType(211);
			if (jumpTask)
			{
				PEDSELF->GetPedIntelligence()->GetTaskManager()->RemoveTask(TASK_PRIORITY_PRIMARY);
			}

			// more removals if needed ever
			//PEDSELF->GetPedIntelligence()->GetTaskManager()->RemoveTask(TASK_PRIORITY_EVENT_RESPONSE_NONTEMP);
			//PEDSELF->GetPedIntelligence()->GetTaskManager()->RemoveTask(TASK_PRIORITY_PHYSICAL_RESPONSE);
			//PEDSELF->GetPedIntelligence()->GetTaskManager()->RemoveTask(TASK_PRIORITY_MAX);
			//PEDSELF->GetPedIntelligence()->GetTaskManager()->RemoveTask(TASK_PRIORITY_PRIMARY);
			...
			if (pTask)
			{
				pTask->SetAsPedTask(PEDSELF, TASK_PRIORITY_MAX);
			}
		}
		else
		{
			// TODO: unload unreferenced blocks later on
			GAME->GetStreaming()->RequestAnimations(pBlock->GetIndex(), 8);
		}
	}
}
...
{
	struct vehicle_info *vinfo = SF->getGame()->vehicleInfoGet(VEHICLE_SELF, 0);
	if (vinfo)
	{
